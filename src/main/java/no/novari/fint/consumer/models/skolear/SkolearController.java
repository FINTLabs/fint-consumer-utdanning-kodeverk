...
@CrossOrigin
@RestController
@RequestMapping(name = "Skolear", value = RestEndpoints.SKOLEAR, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SkolearController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private SkolearCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private SkolearLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Skolear cache is disabled.");
        }
    ...
    @GetMapping
    public SkolearResources getSkolear(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSkolearByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Skolear cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_SKOLEAR, client);
        ...
        Stream<SkolearResource> resources;
    ...
    @PostMapping("/$query")
    public SkolearResources getSkolearByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSkolear(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SkolearResources getSkolearByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_SKOLEAR, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SkolearResources();
    
        ArrayList<SkolearResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SkolearResource>>() {});
    ...
    public SkolearResource getSkolearBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_SKOLEAR, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<SkolearResource> skolear = cacheService.getSkolearBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return skolear.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SkolearResource skolear = objectMapper.convertValue(response.getData().get(0), SkolearResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(skolear);
        }    
    }
