package no.novari.fint.consumer.models.otstatus;
...
public class OtStatusController {
    ...
    public OtStatusResources getOtStatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getOtStatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("OtStatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_OTSTATUS, client);
    ...
    @PostMapping("/$query")
    public OtStatusResources getOtStatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getOtStatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private OtStatusResources getOtStatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_OTSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new OtStatusResources();
    
        ArrayList<OtStatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<OtStatusResource>>() {});
    ...
    public OtStatusResource getOtStatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_OTSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return otstatus.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            return linker.mapAndResetLinks(otstatus);
        }    
    }
