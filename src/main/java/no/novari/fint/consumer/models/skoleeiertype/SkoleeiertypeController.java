package no.novari.fint.consumer.models.skoleeiertype;
...
@Slf4j
@Api(tags = {"Skoleeiertype"})
@CrossOrigin
@RestController
@RequestMapping(name = "Skoleeiertype", value = RestEndpoints.SKOLEEIERTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class SkoleeiertypeController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Skoleeiertype cache is disabled.");
        }
    ...
    @GetMapping
    public SkoleeiertypeResources getSkoleeiertype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getSkoleeiertypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Skoleeiertype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<SkoleeiertypeResource> resources;
    ...
    @PostMapping("/$query")
    public SkoleeiertypeResources getSkoleeiertypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getSkoleeiertype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private SkoleeiertypeResources getSkoleeiertypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_SKOLEEIERTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new SkoleeiertypeResources();
    
        ArrayList<SkoleeiertypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<SkoleeiertypeResource>>() {});
    ...
    public SkoleeiertypeResource getSkoleeiertypeBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<SkoleeiertypeResource> skoleeiertype = cacheService.getSkoleeiertypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return skoleeiertype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            SkoleeiertypeResource skoleeiertype = objectMapper.convertValue(response.getData().get(0), SkoleeiertypeResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(skoleeiertype);
        }    
    }
