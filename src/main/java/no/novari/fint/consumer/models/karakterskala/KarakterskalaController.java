package no.novari.fint.consumer.models.karakterskala;
...
public class KarakterskalaController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Karakterskala cache is disabled.");
        }
    ...
    public KarakterskalaResources getKarakterskala(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKarakterskalaByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Karakterskala cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<KarakterskalaResource> resources;
    ...
    @PostMapping("/$query")
    public KarakterskalaResources getKarakterskalaByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKarakterskala(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KarakterskalaResources getKarakterskalaByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_KARAKTERSKALA, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KarakterskalaResources();
    
        ArrayList<KarakterskalaResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KarakterskalaResource>>() {});
    ...
    public KarakterskalaResource getKarakterskalaBySystemId(
        ...
        if (cacheService != null) {
            ...
            KarakterskalaResource karakterskala = objectMapper.convertValue(response.getData().get(0), KarakterskalaResource.class);
