package no.novari.fint.consumer.models.karakterstatus;
...
@Slf4j
@Api(tags = {"Karakterstatus"})
@CrossOrigin
@RestController
@RequestMapping(name = "Karakterstatus", value = RestEndpoints.KARAKTERSTATUS, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class KarakterstatusController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private KarakterstatusCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Karakterstatus cache is disabled.");
        }
    ...
    @GetMapping
    public KarakterstatusResources getKarakterstatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getKarakterstatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Karakterstatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_KARAKTERSTATUS, client);
        ...
        Stream<KarakterstatusResource> resources;
    ...
    @PostMapping("/$query")
    public KarakterstatusResources getKarakterstatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getKarakterstatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private KarakterstatusResources getKarakterstatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_KARAKTERSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new KarakterstatusResources();
    
        ArrayList<KarakterstatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<KarakterstatusResource>>() {});
    ...
    public KarakterstatusResource getKarakterstatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_KARAKTERSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<KarakterstatusResource> karakterstatus = cacheService.getKarakterstatusBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return karakterstatus.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            KarakterstatusResource karakterstatus = objectMapper.convertValue(response.getData().get(0), KarakterstatusResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(karakterstatus);
        }    
    }
