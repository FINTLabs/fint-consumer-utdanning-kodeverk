package no.novari.fint.consumer.models.eksamensform;
...
import no.novari.fint.model.resource.utdanning.kodeverk.EksamensformResources;
import no.novari.fint.model.utdanning.kodeverk.KodeverkActions;

@Slf4j
@Api(tags = {"Eksamensform"})
@CrossOrigin
@RestController
@RequestMapping(name = "Eksamensform", value = RestEndpoints.EKSAMENSFORM, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class EksamensformController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private EksamensformCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Eksamensform cache is disabled.");
        }
    ...
    @GetMapping
    public EksamensformResources getEksamensform(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getEksamensformByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Eksamensform cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_EKSAMENSFORM, client);
        ...
        Stream<EksamensformResource> resources;
    ...
    @PostMapping("/$query")
    public EksamensformResources getEksamensformByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getEksamensform(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private EksamensformResources getEksamensformByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_EKSAMENSFORM, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new EksamensformResources();
    
        ArrayList<EksamensformResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<EksamensformResource>>() {});
    ...
    public EksamensformResource getEksamensformBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_EKSAMENSFORM, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<EksamensformResource> eksamensform = cacheService.getEksamensformBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return eksamensform.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            EksamensformResource eksamensform = objectMapper.convertValue(response.getData().get(0), EksamensformResource.class);
