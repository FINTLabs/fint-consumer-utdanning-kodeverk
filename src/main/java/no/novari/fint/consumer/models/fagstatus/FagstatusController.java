package no.novari.fint.consumer.models.fagstatus;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Fagstatus", value = RestEndpoints.FAGSTATUS, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FagstatusController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FagstatusCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private FagstatusLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fagstatus cache is disabled.");
        }
    ...
    @GetMapping
    public FagstatusResources getFagstatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFagstatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fagstatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_FAGSTATUS, client);
        ...
        Stream<FagstatusResource> resources;
    ...
    @PostMapping("/$query")
    public FagstatusResources getFagstatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFagstatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FagstatusResources getFagstatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FAGSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FagstatusResources();
    
        ArrayList<FagstatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FagstatusResource>>() {});
    ...
    public FagstatusResource getFagstatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_FAGSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FagstatusResource> fagstatus = cacheService.getFagstatusBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fagstatus.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FagstatusResource fagstatus = objectMapper.convertValue(response.getData().get(0), FagstatusResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(fagstatus);
        }    
    }
