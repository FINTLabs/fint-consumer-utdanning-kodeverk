...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.karakterskala", havingValue = "false", matchIfMissing = true)
public class KarakterskalaCacheService extends CacheService<KarakterskalaResource> {
    ...
    @Scheduled(initialDelayString = Constants.CACHE_INITIALDELAY_KARAKTERSKALA, fixedRateString = Constants.CACHE_FIXEDRATE_KARAKTERSKALA)
    ...
    public void onAction(Event event) {
        ...
        if (KodeverkActions.valueOf(event.getAction()) == KodeverkActions.UPDATE_KARAKTERSKALA) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<KarakterskalaResource>> cacheObjects = data
                    .stream()
