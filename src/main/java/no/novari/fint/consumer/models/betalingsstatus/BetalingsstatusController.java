package no.novari.fint.consumer.models.betalingsstatus;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Betalingsstatus", value = RestEndpoints.BETALINGSSTATUS, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class BetalingsstatusController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private BetalingsstatusCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private BetalingsstatusLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Betalingsstatus cache is disabled.");
        }
    ...
    @GetMapping
    public BetalingsstatusResources getBetalingsstatus(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getBetalingsstatusByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Betalingsstatus cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_BETALINGSSTATUS, client);
        ...
        Stream<BetalingsstatusResource> resources;
    ...
    @PostMapping("/$query")
    public BetalingsstatusResources getBetalingsstatusByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getBetalingsstatus(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private BetalingsstatusResources getBetalingsstatusByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_BETALINGSSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new BetalingsstatusResources();
    
        ArrayList<BetalingsstatusResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<BetalingsstatusResource>>() {});
    ...
    public BetalingsstatusResource getBetalingsstatusBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_BETALINGSSTATUS, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<BetalingsstatusResource> betalingsstatus = cacheService.getBetalingsstatusBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return betalingsstatus.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            BetalingsstatusResource betalingsstatus = objectMapper.convertValue(response.getData().get(0), BetalingsstatusResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(betalingsstatus);
        }    
    }
