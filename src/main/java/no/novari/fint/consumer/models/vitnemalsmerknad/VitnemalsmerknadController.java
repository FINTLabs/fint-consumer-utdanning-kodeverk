...
@Slf4j
@Api(tags = {"Vitnemalsmerknad"})
@CrossOrigin
@RestController
@RequestMapping(name = "Vitnemalsmerknad", value = RestEndpoints.VITNEMALSMERKNAD, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class VitnemalsmerknadController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private VitnemalsmerknadCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Vitnemalsmerknad cache is disabled.");
        }
    ...
    @GetMapping
    public VitnemalsmerknadResources getVitnemalsmerknad(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVitnemalsmerknadByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Vitnemalsmerknad cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_VITNEMALSMERKNAD, client);
        ...
        Stream<VitnemalsmerknadResource> resources;
    ...
    @PostMapping("/$query")
    public VitnemalsmerknadResources getVitnemalsmerknadByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVitnemalsmerknad(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VitnemalsmerknadResources getVitnemalsmerknadByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_VITNEMALSMERKNAD, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VitnemalsmerknadResources();
    
        ArrayList<VitnemalsmerknadResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VitnemalsmerknadResource>>() {});
    ...
    public VitnemalsmerknadResource getVitnemalsmerknadBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VITNEMALSMERKNAD, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<VitnemalsmerknadResource> vitnemalsmerknad = cacheService.getVitnemalsmerknadBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return vitnemalsmerknad.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            VitnemalsmerknadResource vitnemalsmerknad = objectMapper.convertValue(response.getData().get(0), VitnemalsmerknadResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(vitnemalsmerknad);
        }    
    }
