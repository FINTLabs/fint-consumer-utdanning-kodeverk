...
public class TerminController {
    ...
    public TerminResources getTermin(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getTerminByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Termin cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<TerminResource> resources;
    ...
    @PostMapping("/$query")
    public TerminResources getTerminByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getTermin(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private TerminResources getTerminByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_TERMIN, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new TerminResources();
    
        ArrayList<TerminResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<TerminResource>>() {});
    ...
    public TerminResource getTerminBySystemId(
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(termin);
        }    
    }
