...
public class OtEnhetController {
    ...
    public OtEnhetResources getOtEnhet(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getOtEnhetByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("OtEnhet cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_OTENHET, client);
    ...
    @PostMapping("/$query")
    public OtEnhetResources getOtEnhetByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getOtEnhet(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private OtEnhetResources getOtEnhetByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_OTENHET, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new OtEnhetResources();
    
        ArrayList<OtEnhetResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<OtEnhetResource>>() {});
    ...
    public OtEnhetResource getOtEnhetBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_OTENHET, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            return otenhet.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            return linker.mapAndResetLinks(otenhet);
        }    
    }
