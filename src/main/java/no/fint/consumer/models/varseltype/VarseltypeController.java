...
@CrossOrigin
@RestController
@RequestMapping(name = "Varseltype", value = RestEndpoints.VARSELTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class VarseltypeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private VarseltypeCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private VarseltypeLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Varseltype cache is disabled.");
        }
    ...
    @GetMapping
    public VarseltypeResources getVarseltype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVarseltypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Varseltype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_VARSELTYPE, client);
        ...
        Stream<VarseltypeResource> resources;
    ...
    @PostMapping("/$query")
    public VarseltypeResources getVarseltypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVarseltype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VarseltypeResources getVarseltypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_VARSELTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VarseltypeResources();
    
        ArrayList<VarseltypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VarseltypeResource>>() {});
    ...
    public VarseltypeResource getVarseltypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_VARSELTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<VarseltypeResource> varseltype = cacheService.getVarseltypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return varseltype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            VarseltypeResource varseltype = objectMapper.convertValue(response.getData().get(0), VarseltypeResource.class);
