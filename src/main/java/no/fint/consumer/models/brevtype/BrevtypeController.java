...
@CrossOrigin
@RestController
@RequestMapping(name = "Brevtype", value = RestEndpoints.BREVTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class BrevtypeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private BrevtypeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Brevtype cache is disabled.");
        }
    ...
    @GetMapping
    public BrevtypeResources getBrevtype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getBrevtypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Brevtype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_BREVTYPE, client);
        ...
        Stream<BrevtypeResource> resources;
    ...
    @PostMapping("/$query")
    public BrevtypeResources getBrevtypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getBrevtype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private BrevtypeResources getBrevtypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_BREVTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new BrevtypeResources();
    
        ArrayList<BrevtypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<BrevtypeResource>>() {});
    ...
    public BrevtypeResource getBrevtypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_BREVTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<BrevtypeResource> brevtype = cacheService.getBrevtypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return brevtype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            BrevtypeResource brevtype = objectMapper.convertValue(response.getData().get(0), BrevtypeResource.class);
