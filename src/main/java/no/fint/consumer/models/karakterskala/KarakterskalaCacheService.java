...
@Slf4j
@Service
public class KarakterskalaCacheService extends CacheService<FintResource<Karakterskala>> {
    ...
    public KarakterskalaCacheService() {
        super(MODEL, KodeverkActions.GET_ALL_KARAKTERSKALA);
    }
    ...
    public Optional<FintResource<Karakterskala>> getKarakterskalaBySystemId(String orgId, String systemId) {
        return getOne(orgId, (fintResource) -> Optional
                .ofNullable(fintResource)
                .map(FintResource::getResource)
                .map(Karakterskala::getSystemId)
    ...
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Karakterskala>>>() {
        });
    }
}
