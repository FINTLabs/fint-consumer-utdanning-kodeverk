...
import no.fint.model.resource.utdanning.kodeverk.BevistypeResources;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Bevistype", value = RestEndpoints.BEVISTYPE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class BevistypeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private BevistypeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Bevistype cache is disabled.");
        }
    ...
    @GetMapping
    public BevistypeResources getBevistype(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getBevistypeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Bevistype cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_BEVISTYPE, client);
        ...
        Stream<BevistypeResource> resources;
    ...
    @PostMapping("/$query")
    public BevistypeResources getBevistypeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getBevistype(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private BevistypeResources getBevistypeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_BEVISTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new BevistypeResources();
    
        ArrayList<BevistypeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<BevistypeResource>>() {});
    ...
    public BevistypeResource getBevistypeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_BEVISTYPE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<BevistypeResource> bevistype = cacheService.getBevistypeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return bevistype.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            BevistypeResource bevistype = objectMapper.convertValue(response.getData().get(0), BevistypeResource.class);
