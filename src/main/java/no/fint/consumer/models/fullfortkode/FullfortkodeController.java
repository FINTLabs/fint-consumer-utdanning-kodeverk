...
import no.fint.model.resource.utdanning.kodeverk.FullfortkodeResources;
import no.fint.model.utdanning.kodeverk.KodeverkActions;

@Slf4j
@Api(tags = {"Fullfortkode"})
@CrossOrigin
@RestController
@RequestMapping(name = "Fullfortkode", value = RestEndpoints.FULLFORTKODE, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FullfortkodeController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FullfortkodeCacheService cacheService;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fullfortkode cache is disabled.");
        }
    ...
    @GetMapping
    public FullfortkodeResources getFullfortkode(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFullfortkodeByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fullfortkode cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_FULLFORTKODE, client);
        ...
        Stream<FullfortkodeResource> resources;
    ...
    @PostMapping("/$query")
    public FullfortkodeResources getFullfortkodeByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFullfortkode(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FullfortkodeResources getFullfortkodeByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FULLFORTKODE, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FullfortkodeResources();
    
        ArrayList<FullfortkodeResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FullfortkodeResource>>() {});
    ...
    public FullfortkodeResource getFullfortkodeBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_FULLFORTKODE, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FullfortkodeResource> fullfortkode = cacheService.getFullfortkodeBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fullfortkode.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FullfortkodeResource fullfortkode = objectMapper.convertValue(response.getData().get(0), FullfortkodeResource.class);
