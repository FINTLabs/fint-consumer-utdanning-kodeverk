...
public class ElevkategoriController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Elevkategori cache is disabled.");
        }
    ...
    public ElevkategoriResources getElevkategori(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getElevkategoriByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Elevkategori cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<ElevkategoriResource> resources;
    ...
    @PostMapping("/$query")
    public ElevkategoriResources getElevkategoriByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getElevkategori(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ElevkategoriResources getElevkategoriByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_ELEVKATEGORI, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ElevkategoriResources();
    
        ArrayList<ElevkategoriResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ElevkategoriResource>>() {});
    ...
    public ElevkategoriResource getElevkategoriBySystemId(
        ...
        if (cacheService != null) {
            ...
            ElevkategoriResource elevkategori = objectMapper.convertValue(response.getData().get(0), ElevkategoriResource.class);
