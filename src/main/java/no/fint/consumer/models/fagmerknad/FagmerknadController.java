...
import no.fint.model.resource.utdanning.kodeverk.FagmerknadResources;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Fagmerknad", value = RestEndpoints.FAGMERKNAD, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FagmerknadController {

    private static final String ODATA_FILTER_QUERY_OPTION = "$filter=";

    @Autowired(required = false)
    private FagmerknadCacheService cacheService;

    @Autowired
    private FintAuditService fintAuditService;

    @Autowired
    private FagmerknadLinker linker;
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fagmerknad cache is disabled.");
        }
    ...
    @GetMapping
    public FagmerknadResources getFagmerknad(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFagmerknadByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fagmerknad cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_ALL_FAGMERKNAD, client);
        ...
        Stream<FagmerknadResource> resources;
    ...
    @PostMapping("/$query")
    public FagmerknadResources getFagmerknadByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFagmerknad(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FagmerknadResources getFagmerknadByOdataFilter(
        ...
        Event event = new Event(
                ...
                KodeverkActions.GET_FAGMERKNAD, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FagmerknadResources();
    
        ArrayList<FagmerknadResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FagmerknadResource>>() {});
    ...
    public FagmerknadResource getFagmerknadBySystemId(
        ...
        Event event = new Event(orgId, Constants.COMPONENT, KodeverkActions.GET_FAGMERKNAD, client);
        event.setOperation(Operation.READ);
        ...
        if (cacheService != null) {
            ...
            Optional<FagmerknadResource> fagmerknad = cacheService.getFagmerknadBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return fagmerknad.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            FagmerknadResource fagmerknad = objectMapper.convertValue(response.getData().get(0), FagmerknadResource.class);
